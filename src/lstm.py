...
def getData():
    #data = pd.read_csv("Action3dData.csv",usecols=['x']) 
    #data=data['x', 'y','z']
   ...
   # data.head(4)
    ...
    if args.db=='action':
        ...
        #print(data.shape)
        #for p in data:
            #np.array(datanp.append(p.tolist()))
        #print('datanp',d[:3,:])
        #print('datanp',type(d))
    if args.db=='dance':
...
The reason for this is that neural networks are stochastic, meaning that a different specific model will have the same result when training the same model configuration on the same data.'''
    ...
    #fonction de cout pour la régression
    #cost_function = keras.losses.mean_squared_error
    
    # algorithme d'optimisation 
    #optimizer_algo = keras.optimizers.Adam(lr=0.001)
    
    
    # couche en entrée
    #input_layer = keras.layers.Input(batch_shape=[batch_size,x_train.shape[0],x_train.shape[2]])

    
    # première couche cachée d'un LSTM contenant nb_neurons 
    # stateful = True pour dire que l'output d'un batch est utilisé comme input du
    # second batch etc ... : Récurrence sur les batch quand les batch_size = 1 
    #hidden_layer_1 = keras.layers.LSTM(units=nb_neurons, stateful=True)(input_layer)
    
    # dernière couche de prédiction linéaire qui prédit une seule valeur réelle
    #output_layer = keras.layers.Dense(units=nb_classes)(hidden_layer_1)
    ...
    #nb_epoch = 10 
    #model.fit(x_train, y_train, epochs=1, batch_size=1, verbose=True, shuffle=False)
...
#        model = creat_model(input_shape, num_class)
        ...
        #optimizer = keras.optimizers.Adam(lr=0.005, clipnorm=1.0)
        #model.compile(loss='categorical_crossentropy', optimizer=optimizer, metrics=['accuracy'])
        #model.fit(x_train, y_train, epochs=1,batch_size=1, verbose=True,shuffle=False)
        
        #history = model.fit(x_train,y_train,batch_size=mini_batch_size,epochs=nb_epochs,verbose=False,validation_split=percentage_of_train_as_validation,callbacks=[model_checkpoint])
...
#            model.reset_states()
        ...
        #scores = get_activation(model, x_test, y_test, 'pred.txt', VA=10, par=9)
    
        #results.append(round(scores, 2))
    
    """## Mesurer l'erreur RMSE (Root Mean Squared Error)"""
    # performance
